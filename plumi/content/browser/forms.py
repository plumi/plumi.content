...
class ScalarVideoAddForm(form.SchemaForm):
    grok.name('publish_video_scalar')
    grok.require('cmf.AddPortalContent')
    grok.context(IFolderish)
    ...
    def update(self):
        ...
        self.request.set('disable_border', True)
        # call the base class version - this is very important!
        super(ScalarVideoAddForm, self).update()
    ...
    def handleApply(self, action):
        ...
        if data.get('CommentaryDescription') and data.get('CommentaryTitle'):         
            pm = getToolByName(obj, 'portal_membership')
            home = pm.getHomeFolder()
            title = unicode(strip_tags(data.get('CommentaryTitle')))[:200]
            body = data.get('CommentaryDescription')
            ...
            try:
                ...
                commentary_obj = target_folder[new_id]
                commentary_obj.textCommentary = RichTextValue(unicode(strip_tags(body))[:20000], 'text/plain', 'text/plain', 'utf-8')
                RelatedItems = obj.getRelatedItems()
                RelatedItems.append(commentary_obj)
